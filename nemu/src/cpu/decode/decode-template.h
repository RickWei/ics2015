...
make_helper(concat(decode_si_, SUFFIX)) {
	...
	if(DATA_BYTE==4){
		...
		op_src->imm =(uint16_t) (int8_t) instr_fetch(eip, DATA_BYTE);
	}
	else{
		op_src->imm =(uint32_t) (int8_t) instr_fetch(eip, DATA_BYTE);
	}
...
make_helper(concat(decode_rml2r_,SUFFIX)){
	op_src->size = 2;
	int len = read_ModR_M(eip, op_src, op_dest);
	op_dest->val = REG(op_dest->reg);
	return len;
...
make_helper(concat(decode_jrm_,SUFFIX)){
	op_src->size = DATA_BYTE;
	read_ModR_M(eip, op_src, op_src2);
	op_src2->val = REG(op_src2->reg);
	return -1;
}
