...
int find_obj(char* my_obj){
	int i;
	for(i = 0; i < nr_symtab_entry; i++) {
		if(!strcmp(strtab+symtab[i].st_name,my_obj))
			return symtab[i].st_value;
	}
	return 0;
}

int find_funaddr(int value);

char *find_funname(int value){
	int i;
	for(i = 0; i<nr_symtab_entry;i++){
		if(symtab[i].st_value==find_funaddr(value))
...
int find_funaddr(int value){
	int i;
	for(i = 0;i<nr_symtab_entry;i++){
		if(ELF32_R_TYPE(symtab[i].st_info)==18)
			if(symtab[i].st_size+symtab[i].st_value>=value)
